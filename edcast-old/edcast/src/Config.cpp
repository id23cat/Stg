...
void CConfig::GlobalsToDialog(edcastGlobals *g) {	
    ...
    */
	...
	basicSettings->m_BitrateCtrl.SetWindowText(buf);
    sprintf(buf, "%d", getCurrentChannels(g));
    basicSettings->m_Channels = buf;
	basicSettings->m_ChanelsCtrl.SetWindowText(buf);
    sprintf(buf, "%d", getCurrentSamplerate(g));
    basicSettings->m_Samplerate = buf;
	basicSettings->m_SampleRateCtrl.SetWindowText(buf);
    ...
    if (g->gAACFlag) {
		...
		basicSettings->m_QualityCtrl.SetWindowText(g->gAACQuality);
    }
    if (g->gAACPFlag) {
        basicSettings->m_EncoderType = "AAC Plus";
        basicSettings->m_Quality = "";
		basicSettings->m_QualityCtrl.SetWindowText("");
    }
    if (g->gLAMEFlag) {
        basicSettings->m_EncoderType = "MP3 Lame";
        basicSettings->m_Quality = g->gOggQuality;
		basicSettings->m_QualityCtrl.SetWindowText(g->gOggQuality);
    }
    if (g->gOggFlag) {
	...
	basicSettings->m_MountPointCtrl.SetWindowText(g->gMountpoint);
	basicSettings->m_Password = g->gPassword;
	basicSettings->m_PasswordCtrl.SetWindowText(g->gPassword);

	sprintf(buf, "%d", g->gReconnectSec);
    basicSettings->m_ReconnectSecs = buf;
	basicSettings->m_ReconnectSecsCtrl.SetWindowText(buf);
    basicSettings->m_ServerIP = g->gServer;
	basicSettings->m_ServerIpCtrl.SetWindowText(g->gServer);
    basicSettings->m_ServerPort = g->gPort;
	basicSettings->m_ServerPortCtrl.SetWindowText(g->gPort);
	...
	basicSettings->m_JointStereoCtrl.SetCheck(BST_CHECKED);
    ...
    */
	...
	ypSettings->m_PublicCtrl.SetCheck(ypSettings->m_Public);
    ypSettings->m_StreamDesc = g->gServDesc;
	ypSettings->m_StreamDescCtrl.SetWindowText(g->gServDesc);
    ypSettings->m_StreamName = g->gServName;
	ypSettings->m_StreamNameCtrl.SetWindowText(g->gServName);
    ypSettings->m_StreamGenre = g->gServGenre;
	ypSettings->m_StreamGenreCtrl.SetWindowText(g->gServGenre);
    ypSettings->m_StreamURL = g->gServURL;
	ypSettings->m_StreamURLCtrl.SetWindowText(g->gServURL);
    ypSettings->m_StreamAIM = g->gServAIM;
	ypSettings->m_StreamAIMCtrl.SetWindowText(g->gServAIM);
    ypSettings->m_StreamICQ = g->gServICQ;
	ypSettings->m_StreamICQCtrl.SetWindowText(g->gServICQ);
    ypSettings->m_StreamIRC = g->gServIRC;
	ypSettings->m_StreamIRCCtrl.SetWindowText(g->gServIRC);
