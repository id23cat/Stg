...
CMainWindow::CMainWindow(CWnd *pParent /* NULL */ ) :
	...
	bMinimized_(false),
	pTrayIcon_(0),
	nTrayNotificationMsg_(RegisterWindowMessage(kpcTrayNotificationMsg_))/* , fileNameString(_T(""))*/
	{
...
void CMainWindow::DoDataExchange(CDataExchange *pDX) {
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_MESSAGE, messageFileEdit);

	DDX_Control(pDX, IDC_HIDEMESSAGE, hideButton);
}
...
//{{AFX_MSG_MAP(CMainWindow)
	...
	ON_BN_CLICKED(IDC_OPENMESSAGE, &CMainWindow::OnBnClickedOpenmessage)
	ON_BN_CLICKED(IDC_HIDEMESSAGE, &CMainWindow::OnBnClickedHidemessage)
	ON_EN_CHANGE(IDC_MESSAGE, &CMainWindow::OnEnChangeMessageFile)
END_MESSAGE_MAP()
...
void CMainWindow::OnBnClickedOpenmessage()
{
	// TODO: Add your control notification handler code here
	CFileDialog dlgFile(TRUE);
	dlgFile.DoModal();
	CString fname = dlgFile.GetPathName();
	messageFileEdit.SetWindowText(fname);
	messageFileEdit.UpdateData();
	
}

void CMainWindow::OnBnClickedHidemessage()
{
	// TODO: Add your control notification handler code here
}

void CMainWindow::OnEnChangeMessageFile()
{
	...
	// TODO:  Add your control notification handler code here
	int length = messageFileEdit.LineLength(0);
	char* str1 = new char[length];
	char* str2 = new char[length];
	messageFileEdit.GetLine(0,str1);	
	int len = length;
	...
	CFileStatus stat;
	CFile::GetStatus(str2,stat);
	if(PathFileExists(str2))
		stegoEnc.SetMessageFile(str2);
 }
