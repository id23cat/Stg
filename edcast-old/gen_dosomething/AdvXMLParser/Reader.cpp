...
// Parse the document
{
    ...
    auto_ptr<Document> pDocument(Document::Parse(self));
...
{
    // Get the root element
    auto_ptr<Element> pRoot(Element::Parse(parser, self));
    if(NULL == pRoot.get())
...
// [27] Misc ::= Comment | PI | S
{
    ...
    auto_ptr<Comment> pComment(Comment::Parse(parser, self));
    ...
    auto_ptr<Pi> pPI(Pi::Parse(parser, self));
    if(NULL != pPI.get())
    {
        Add(pPI.release());
        return(true);
...
{
    bool bIsChoice;
    if(!ParseChoiceSeq(parser, bIsChoice))
        return(false);
...
bool Dtd::ParseChoiceSeq(Parser& parser, bool& bIsChoice)
...
// [48] cp ::= (Name | choice | seq) ('?' | '*' | '+')? 
{
    ...
    if(!parser.ParseName(strName) && !ParseChoiceSeq(parser, bIsChoice))
        return(false);
...
void Element::ParseReference(Parser& parser)
{
    ...
    auto_ptr<Reference> pReference(Reference::Parse(parser, self));
    if(pReference.get() == NULL) // Failed ?
        parser.SyntaxError(ERROR_INVALID_REFERENCE);
...
// Comment begins with   '<!--'
{
    ...
    // Try to read a CDATA
    auto_ptr<CData> pCData(CData::Parse(parser, self));
    if(NULL != pCData.get())
    {
        // Add it to the element
        Add(pCData.release());
    ...
    auto_ptr<Element> pElement(Element::Parse(parser, self));
