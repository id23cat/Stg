  ...
  if (doc) {
	...
	for (cur_node = root_element->children; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp((char *)cur_node->name, "entry")) {
				...
				for (entry_node = cur_node->children; entry_node; entry_node = entry_node->next) {
					if (entry_node->type == XML_ELEMENT_NODE) {
						if (!strcmp((char *)entry_node->name, "server_name")) {
							if (!p->name) {
								xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
								if (pValue) {
						...
						if (!strcmp((char *)entry_node->name, "server_desc")) {
							if (p->name) {
								free(p->name);
								p->name = NULL;
							}
						...
						if (!strcmp((char *)entry_node->name, "listen_url")) {
							xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
							if (pValue) {
								p->filename = strdup((char *)pValue);
								xmlFree(pValue);
							}
						}
						if (!strcmp((char *)entry_node->name, "server_type")) {
							xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
							if (pValue) {
								p->type = strdup((char *)pValue);
								xmlFree(pValue);
							}
						}
						if (!strcmp((char *)entry_node->name, "bitrate")) {
							xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
							if (pValue) {
						...
						if (!strcmp((char *)entry_node->name, "genre")) {
							xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
							if (pValue) {
						...
						if (!strcmp((char *)entry_node->name, "current_song")) {
							xmlChar *pValue = xmlNodeListGetString(doc, entry_node->xmlChildrenNode, 1);
							if (pValue) {
...
#define RETIFNZ(v) if ((v)<0) return use_dir?1:-1; if ((v)>0) return use_dir?-1:1;
  ...
  for (x = 0; x < 4; x ++)
  {
    ...
    {
      int v=STRCMP_NULLOK(a->bitrate,b->bitrate);
      RETIFNZ(v)
      use_by=COL_FILENAME;     
    }
    else if (use_by == COL_FILENAME) // length -> artist -> album -> track
    {
  ...
  switch (uMsg)
  {
    ...
    case WM_INITDIALOG:
      ...
      *(void **)&cr_resize=(void*)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,33,ML_IPC_SKIN_WADLG_GETFUNC);
      ...
      m_list.AddCol("Stream Type",100);
      m_list.AddCol("Stream Name",200);
      m_list.AddCol("Genre",100);
      m_list.AddCol("Now Playing",200);
      m_list.AddCol("Bitrate",64);
      m_list.AddCol("Filename",200);
      ListView_SetTextColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMFG):RGB(0xff,0xff,0xff));
      ListView_SetBkColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMBG):RGB(0x00,0x00,0x00));
      ListView_SetTextBkColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMBG):RGB(0x00,0x00,0x00));
