...
xmlDocPtr URLFetch::Perform(int flag, char *type, char *param)
{
	int retcode = 0;
	...
	char	URL[2046] = "";
	if (flag == 1) {
		sprintf(URL, "http://www.shoutcast.com/sbin/newxml.phtml?%s=", type);
		char *pencoded = NULL;
		pencoded = curl_escape(param, strlen(param));
		strcat(URL, pencoded);
		curl_free(pencoded);
	}
	if (flag == 2) {
		strcpy(URL, "http://www.shoutcast.com/sbin/newtvlister.phtml?alltv=1");
	}
