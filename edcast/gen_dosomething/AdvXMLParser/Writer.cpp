...
// Output the text after replacing each quote by "&quote;".
// (when XML is generated, we always use " (quotes) so every
// quote in the text has to be escaped).
//
...
// the text before generation.
{
    ...
    while(nPosition != String::npos)
    {
        ...
        nPosition = strText.find(TEXT('\"'), nPreviousPosition);
    }

    if(xml.MustPreserve() || strText != TEXT(" "))
