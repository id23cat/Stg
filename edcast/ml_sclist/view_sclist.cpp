...
/*
** This ML plugin was derived heavily from the example ML plugin ml_ex...
**                                                oddsock@oddsock.org
**/
...
#define CONFIG_SEC "ml_icecastlist"

void loadSongList();
static void updateList(char *extra);
// columns in our big treeview

#define COL_TYPE 0
#define COL_NAME 1
#define COL_GENRE 2
#define COL_NOW_PLAYING 3
#define COL_BITRATE 4
...
void SongsToItemList(itemRecordList *p, int all)
{
  ...
  for (x = 0 ; x < l; x ++)
  {
    ...
    p->Items[p->Size].title=s->name;
    p->Items[p->Size].genre=s->genre;
    p->Items[p->Size].comment=s->now_playing;
...
static void playFiles(int enqueue, int all)
{
  ...
  if (obj.Size)
  {
	mlSendToWinampStruct s={ML_TYPE_FILENAMES,(void*)(obj.Items->filename),!!enqueue};
...
//    if (p->Items[x].type) s->album=strdup(p->Items[x].type);
    if (p->Items[x].title) s->name=strdup(p->Items[x].title);
    if (p->Items[x].genre) s->genre=strdup(p->Items[x].genre);
    if (p->Items[x].comment) s->now_playing=strdup(p->Items[x].comment);
    //if (p->Items[x].bitrate) s->genre=strdup(p->Items[x].bitrate);
...
void loadSongListThread(void *dummy);
int inRefresh = 0;
char	currentSearch[1024] = "";
int		currentType = 1;
char	currentSearchVariable[255] = "";

void loadSongList()
{
    if (inRefresh == 0) {
        _beginthread(loadSongListThread, 0, NULL);
    }

}
int progressCallback(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow) {
  char buffer[256] = "";
  sprintf(buffer, "Retrieving Stations...(%d of %d bytes)", (long)dlnow, (long)dltotal);
  updateList(buffer);
  return 0;
}
...
void loadSongListThread(void *dummy)
{
  ...
  xmlDocPtr doc = fetchit.Perform(currentType, currentSearchVariable, currentSearch);
  ...
  if (doc) {
	...
	for (cur_node = root_element->children; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp((char *)cur_node->name, "station")) {
				Song *p=(Song *)calloc(sizeof(Song),1);

				xmlChar *pValue =NULL;
				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"name")) != NULL) {
					p->name = strdup((char *)pValue);
					xmlFree(pValue);
					if (currentType == 2) {
						p->type = strdup((const char *)pNSV);
					}
				}

				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"id")) != NULL) {
					p->filename = (char *)malloc(1024);
					memset(p->filename, '\000', 1024);
					if (currentType == 2) {
						sprintf(p->filename, "http://www.shoutcast.com/sbin/tunein-tvstation.pls?id=%s", pValue);
					}
					else {
						sprintf(p->filename, "http://www.shoutcast.com/sbin/tunein-station.pls?id=%s", pValue);
					}
					xmlFree(pValue);
				}
				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"mt")) != NULL) {
					if (!strcmp((const char *)pValue, (const char *)"audio/mpeg")) {
						p->type = strdup((const char *)pMP3);
					}
					if (!strcmp((const char *)pValue, (const char *)"audio/aacp")) {
						p->type = strdup((const char *)pAACP);
					}
					xmlFree(pValue);
				}
				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"br")) != NULL) {
					p->bitrate = strdup((char *)pValue);
					xmlFree(pValue);
				}
				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"genre")) != NULL) {
					p->genre = strdup((char *)pValue);
					xmlFree(pValue);
				}
				if ((pValue = (unsigned char *)xmlGetProp(cur_node, (const unsigned char *)"ct")) != NULL) {
					p->now_playing = strdup((char *)pValue);
					xmlFree(pValue);
				}
...
//  parsequicksearch(filteritems,filterstr);
  ...
  for(int i=0;i<m_songs.GetSize();i++)
  {
    ...
    if (*p)
    {
      while (*p)
      {
        ...
        if (!in_string(s->type,p) && !in_string(s->name,p) && !in_string(s->genre,p) && !in_string(s->now_playing,p) && !in_string(s->filename,p) && !in_string(s->bitrate,p))
          break;
  ...
  if (extra) {
    wsprintf(tmp,"%s",extra);
  }
  else {
      if (m_songs.GetSize() != m_songs_sorted->GetSize())
        wsprintf(tmp,"Found: %d items",
        m_songs_sorted->GetSize());
      else
        wsprintf(tmp,"%d items",m_songs.GetSize());
  }
...
static int NUMCMP(const char *pa, const char *pb)
{
  if (!pa) pa="0";
  else SKIP_THE_AND_WHITESPACE(pa)
...
#define RETIFNZ(v) if ((v)<0) return use_dir?1:-1; if ((v)>0) return use_dir?-1:1;
  ...
  for (x = 0; x < 4; x ++)
  {
    ...
    if (use_by == COL_TYPE) // title -> artist -> album -> track
    {
      int v=STRCMP_NULLOK(a->type,b->type);
      RETIFNZ(v)
      use_by=COL_NAME;
    }
    else if (use_by == COL_NAME) // artist -> album -> track -> title
    {
      int v=STRCMP_NULLOK(a->name,b->name);
      RETIFNZ(v)
      use_by=COL_GENRE;
    }
    else if (use_by == COL_GENRE) // album -> track -> title -> artist
    {
      int v=STRCMP_NULLOK(a->genre,b->genre);
      RETIFNZ(v)
      use_dir=0;
      use_by=COL_NOW_PLAYING;
    }
    else if (use_by == COL_NOW_PLAYING) // genre -> artist -> album -> track
    {
      int v=STRCMP_NULLOK(a->now_playing,b->now_playing);
      RETIFNZ(v)
      use_by=COL_BITRATE;
    }
    else if (use_by == COL_BITRATE) // track -> title -> artist -> album
    {
      int v=NUMCMP(a->bitrate,b->bitrate);
      RETIFNZ(v)
      use_by=COL_TYPE;
    }
  ...
  switch (uMsg)
  {
    ...
    case WM_INITDIALOG:
      ...
      *(void **)&cr_resize=(void*)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,33,ML_IPC_SKIN_WADLG_GETFUNC);
      ...
      m_list.AddCol("Type",50);
      m_list.AddCol("Stream Name",300);
      m_list.AddCol("Genre",100);
      m_list.AddCol("Now Playing",300);
      m_list.AddCol("Bitrate",64);
      ListView_SetTextColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMFG):RGB(0xff,0xff,0xff));
      ListView_SetBkColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMBG):RGB(0x00,0x00,0x00));
      ListView_SetTextBkColor(m_list.getwnd(),wad_getColor?wad_getColor(WADLG_ITEMBG):RGB(0x00,0x00,0x00));
      ...
      if (l->idFrom==IDC_LIST)
      {
        ...
        else if (l->code == LVN_ODFINDITEM) // yay we find an item (for kb shortcuts)
        {
          ...
          while (cnt-->0)
          {
            ...
            switch (g_sortcol)
            {
              case COL_TYPE: name=thissong->type; break;
              case COL_NAME: name=thissong->name; break;
              case COL_GENRE: name=thissong->genre; break;
              case COL_NOW_PLAYING: name=thissong->now_playing; break;
              case COL_BITRATE: name=thissong->bitrate; break;
            }
        ...
        else if (l->code == LVN_GETDISPINFO)
        {
          ...
          if (lpdi->item.mask & (LVIF_TEXT|/*LVIF_IMAGE*/0)) // we can always do images too :)
          {
            if (lpdi->item.mask & LVIF_TEXT)
            {
              ...
              {
                case COL_TYPE: nameptr=thissong->type; break;
                case COL_NAME: nameptr=thissong->name; break;
                case COL_GENRE: nameptr=thissong->genre; break;
                case COL_NOW_PLAYING: nameptr=thissong->now_playing; break;
                case COL_BITRATE: nameptr=thissong->bitrate; break;
    ...
    case WM_COMMAND:
      switch(LOWORD(wParam)) 
      {
	    ...
	    case IDC_BUTTON_TV:
			strcpy(currentSearch, "");
			strcpy(currentSearchVariable, "");
			currentType = 2;
			loadSongList();
        ...
        break;
	    case IDC_BUTTON_TOP500:
			strcpy(currentSearch, "Top500");
			strcpy(currentSearchVariable, "genre");
			currentType = 1;
			loadSongList();
			//GetPrivateProfileString(CONFIG_SEC,"lastfilter","",buf,sizeof(buf),conf_file);
			//SetDlgItemText(hwndDlg,IDC_QUICKSEARCH,buf); // automatically updates the list via EN_CHANGE
	    ...
	    case IDC_BUTTON_SEARCH:
			...
			buf[255]=0;
			strcpy(currentSearch, buf);
			strcpy(currentSearchVariable, "search");
			currentType = 1;
			loadSongList();
			//GetPrivateProfileString(CONFIG_SEC,"lastfilter","",buf,sizeof(buf),conf_file);
			//SetDlgItemText(hwndDlg,IDC_QUICKSEARCH,buf); // automatically updates the list via EN_CHANGE
    ...
    case WM_PAINT:
      {
		...
		*/
        ...
        if (wad_DrawChildWindowBorders) wad_DrawChildWindowBorders(hwndDlg,tab,1);
      }
    return 0;
    case WM_DESTROY:
      //clearSongList();
...
int onTreeItemClick(int param, int action, HWND hwndParent) // if param is not yours, return 0
{
  if (action == ML_ACTION_RCLICK) 
  {
    ...
    {
      case ID_ABOUT:
        MessageBox(hwndParent,"Icecast Stream Directory Browser by oddsock@oddsock.org","About",MB_OK);
      break;
    }
  }
