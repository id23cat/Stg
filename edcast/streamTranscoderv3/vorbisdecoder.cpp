...
int handle_vorbis_output(devorb *devorb)
{
	...
	for (i=0;i<(int)nsamples;i++) {
		...
		samples[samplecount] = fsample;//(float)(devorb->pcm[0][i]);
		samplecount++;

		if (nchannels == 2) {
			fsample = (float)(devorb->pcm[1][i]);
			if (fsample > 1.0) {
				fsample = 1.0;
			}
			if (fsample < -1.0) {
			...
			samples[samplecount] = fsample;//(float)(devorb->pcm[1][i]);
			samplecount++;
		}
	}
