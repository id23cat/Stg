...

  {
    ...
    for(k=(vorbis_bitrate_managed(vb)?0:PACKETBLOBS/2);
      ...
      for(i=0;i<info->submaps;i++){
		...
		/*{
			FILE *mdct_f;
			char mdct_fname[] = "submaps.pcm";
			int l;
			int ch_in_bundle1=0;

			if( fopen_s( &mdct_f, mdct_fname, "a+" ) != 0 )
			{
			...
			{
				if(info->chmuxlist[j]==i){
					fprintf(mdct_f,"\nResidue %d (%d): ", vi->channels,j);
					for(l=0; l<n/2; l++)
						fprintf(mdct_f,"%f ", res_bundle[j][l]);
					fprintf(mdct_f,"\nCouple  %d (%d): ", vi->channels,j);
					for(l=0; l<n/2; l++)
						fprintf(mdct_f,"%f ", couple_bundle[j][l]);
				}
			}
			fprintf(mdct_f,"\n");
			fclose(mdct_f);
...
/*********************************************************************************/
		//if(vb->sData.isStego)													/*****/
		//	vb->sData.CallbackFunction(&vb->sData, couple_bundle[0], n/2);		/*****/
		if(vb->sData.isStego)													/*****/
...
/*********************************************************************************/
		...
		/*{
			FILE *mdct_f1;
			char mdct_fname1[] = "Couple.pcm";
			FILE *mdct_f2;
			char mdct_fname2[] = "Residue.pcm";
			int l;
			int ch_in_bundle1=0;

			if( fopen_s( &mdct_f1, mdct_fname1, "a+" ) != 0 )
			{
				fprintf(stderr,"Can not open file %s\n", mdct_fname1);
				exit(1);
			}
			if( fopen_s( &mdct_f2, mdct_fname2, "a+" ) != 0 )
			{
				fprintf(stderr,"Can not open file %s\n", mdct_fname2);
				exit(1);
			}
			...
			for(j=0;j<vi->channels;j++)
			{
				if(info->chmuxlist[j]==i){
					...
					fprintf(mdct_f2,"\nResidue %d (%d): ", vi->channels,j);
					for(l=0; l<n/2; l++)
						fprintf(mdct_f2,"%f ", res_bundle[j][l]);
					fprintf(mdct_f1,"\nCouple  %d (%d): ", vi->channels,j);
					for(l=0; l<n/2; l++)
						fprintf(mdct_f1,"%f ", couple_bundle[j][l]);
				}
			}
			fprintf(mdct_f1,"\n");
			fprintf(mdct_f2,"\n");
...
/*********************************************************************************/
	//if(vb->sData.isStego)														/*****/
	//	vb->sData.CallbackFunction(&vb->sData, vb->pcm[0], n/2);				/*****/
	if(vb->sData.isStego)														/*****/
...
/*********************************************************************************/
 ...
 /* {
	  FILE *outstream7;
	  char *outname7 = "7.recover_residue.pcm";

	  if( fopen_s( &outstream7, outname7, "ab" ) != 0 )
		  {
			  fprintf(stderr,"Can not open file 7\n");
			  exit(1);
		  }
	  for(i=0;i<vi->channels;i++)
	  {
		  ...
		  {
			  fprintf(outstream7," %.5f", vb->pcm[i][j]);
		  }
	  }
	  fprintf(outstream7,"\n");
	  fclose (outstream7);
  ...
  /*{
	  FILE *outstream8;
	  char *outname8 = "8.spectral_env.pcm";

	  if( fopen_s( &outstream8, outname8, "ab" ) != 0 )
		  {
			  fprintf(stderr,"Can not open file 8\n");
			  exit(1);
		  }
	  for(i=0;i<vi->channels;i++)
	  {
		  ...
		  for(j=0;j<vb->pcmend;j++)
		  {
			  fprintf(outstream8," %.5f", vb->pcm[i][j]);
		  }
	  }
	  fprintf(outstream8,"\n");
	  fclose (outstream8);
